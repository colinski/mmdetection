...
class TagDETRHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is TagDETRHead):
        ...
        self.loss_tag = nn.BCEWithLogitsLoss()
    ...
    def _init_layers(self):
        ...
        self.tag_ffn = FFN(
            ...
            dropout=0.0,
            add_residual=False)
        self.fc_tag = Linear(self.embed_dims, 80)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is TagDETRHead:
            convert_dict = {
    ...
    def loss(self,
        ...
        gt_counts = torch.cat(gt_counts, dim=0)
        gt_tag_targets = (gt_counts != 0).float()
        loss_tag = self.loss_tag(self.tag_preds[-1], gt_tag_targets)
        
        loss_dict['loss_tag'] = self.loss_tag(self.tag_preds[-1], gt_tag_targets)
        for i in range(len(self.tag_preds) - 1):
            loss_dict[f'd{i}.loss_tag'] = self.loss_tag(self.tag_preds[i], gt_tag_targets)
