...
class CountDETRHead(AnchorFreeHead):
    ...
    def loss(self,
        ...
        losses_cls, losses_conf, losses_bbox, losses_iou = multi_apply(
        ...
        loss_dict['loss_conf'] = losses_conf[-1]
        ...
        num_dec_layer = 0
        for loss_cls_i, loss_conf_i, loss_bbox_i, loss_iou_i in zip(losses_cls[:-1],
                                                       losses_conf[:-1],
                                                       losses_bbox[:-1],
                                                       losses_iou[:-1]):
            loss_dict[f'd{num_dec_layer}.loss_cls'] = loss_cls_i
            loss_dict[f'd{num_dec_layer}.loss_conf'] = loss_conf_i
    ...
    def loss_single(self,
        ...
        labels = labels[~is_bg]
        label_weights = label_weights[~is_bg]
        ...
        return loss_cls, loss_conf, loss_bbox, loss_iou
    ...
    def _get_bboxes_single(self,
        ...
        if self.loss_cls.use_sigmoid:
            ...
            conf_score = cls_score[..., -1].sigmoid()
            cls_score = cls_score[..., :-1]
            scores, det_labels = F.softmax(cls_score, dim=-1).max(-1)
            scores = scores * (1.0 - conf_score)
